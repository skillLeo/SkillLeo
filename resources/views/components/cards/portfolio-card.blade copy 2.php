    ...
    $tags     = $p['tags'] ?? ($p['tag_names'] ?? []);   // <-- prefer relation, fallback to meta
    $category = $p['category'] ?? null;

    $tagSlugs = array_map(fn($t) => Str::slug($t), $tags);
    $host = null;
    if ($link) {
        try {
    ...
    <div class="flex-link">
        ...
        <div class="lpc-skills">
            ...
            <span>{{ implode(', ', array_slice($tags, 0, 2)) }}{{ count($tags) > 2 ? ' +' . (count($tags) - 2) . ' more' : '' }}</span>
        </div>
    @endif
    </div>
