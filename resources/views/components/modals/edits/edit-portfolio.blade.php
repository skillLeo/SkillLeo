...
<x-modals.edits.base-modal id="editPortfolioModal" title="Portfolio Projects" size="lg">
    <form id="portfolioModalForm" method="POST" action="{{ route('tenant.portfolio.update',$username) }}">
        @csrf
        @method('PUT')
        ...
        <div class="pf-sort-controls">
            <div class="pf-sort-left">
                ...
                <div class="pf-sort-tabs">
                    <button type="button" class="pf-sort-tab active" data-sort="custom" onclick="setSortMode('custom')">
                    ...
                    <button type="button" class="pf-sort-tab" data-sort="newest" onclick="setSortMode('newest')">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="pf-sort-hint" id="sortHintCustom">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M12 5v14M5 12l7 7 7-7"/>
                </svg>
                <span>Drag to reorder projects</span>
            </div>
            <div class="pf-sort-hint" id="sortHintNewest" style="display: none;">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        {{-- Projects List (UNIQUE ID) --}}
        <div class="pf-list" id="pfModalList"></div>

        {{-- Empty State (UNIQUE ID) --}}
        <div class="pf-empty" id="pfModalEmpty">
    ...
    <x-slot:footer>
        ...
        <button type="submit" form="portfolioModalForm" class="btn-modal btn-primary" id="savePortfoliosBtn">
            Save Changes
        </button>
    </x-slot:footer>
...
<script>
    ...
    (function initPortfolioModal(){
        ...
        const el = {
            list:     document.getElementById('pfModalList'),   // <-- fixed
            empty:    document.getElementById('pfModalEmpty'),  // <-- fixed
        ...
        window.setSortMode = function(mode) {
            ...
            document.querySelectorAll('.pf-sort-tab').forEach(tab => {
                tab.classList.toggle('active', tab.dataset.sort === mode);
            });
            document.getElementById('sortHintCustom').style.display = mode === 'custom' ? 'flex' : 'none';
            document.getElementById('sortHintNewest').style.display = mode === 'newest' ? 'flex' : 'none';
            if (mode === 'newest') {
                portfolios.sort((a, b) => (b.db_id || 0) - (a.db_id || 0));
            }
            render();
        };
        ...
        function loadExisting() {
            portfolios = existingPortfolios.map((p, i) => ({
                ...
                skill_ids: Array.isArray(p.skill_ids) ? p.skill_ids : [], // keep if you map skills
                db_id: p.id || null,
        ...
        function render() {
            if (portfolios.length === 0) {
                ...
                document.querySelector('.pf-sort-controls').style.display = 'none';
            } else {
                el.empty.style.display = 'none';
                el.list.style.display  = 'block';
                document.querySelector('.pf-sort-controls').style.display = 'flex';
                el.list.innerHTML = portfolios.map(p =>
                    editingId === p.id ? renderEdit(p) : renderPreview(p)
                ).join('');

                if (currentSortMode === 'custom' && editingId === null) {
                    attachDragEvents();
                }
            }

            // keep data in submission order
            ...
            if (editingId !== null) {
                setTimeout(() => {
                    const inp = document.getElementById(`title-${editingId}`);
                    if (inp) inp.focus();
                }, 0);
            }
        }
        ...
        function attachDragEvents() {
            const items = el.list.querySelectorAll('.pf-preview');
        ...
        function handleDrop(e) {
            ...
            const newOrder = [];
            items.forEach(item => {
                const id = parseInt(item.id.replace('port-', ''));
                const portfolio = portfolios.find(p => p.id === id);
                if (portfolio) newOrder.push(portfolio);
            });
            portfolios = newOrder;
            // update hidden field right away
            const dataToSave = portfolios.map(p => ({
                db_id: p.db_id,
                title: p.title,
                description: p.description,
        ...
        function handleDragEnd() {
            ...
            el.list.querySelectorAll('.pf-preview').forEach(item => item.classList.remove('over'));
        }
        ...
        function renderPreview(p) {
            ...
            const skillNames = (p.skill_ids || [])
                ...
                .filter(Boolean).map(s => s.name).slice(0,3);
            const isDraggable = currentSortMode === 'custom' && editingId === null;

            return `
            <div class="pf-preview ${isDraggable ? 'draggable' : ''}" id="port-${p.id}">
                ${isDraggable ? `<div class="pf-drag-handle" title="Drag to reorder">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="18" x2="21" y2="18"/>
                    </svg></div>` : ''}
                <div class="pf-preview-main">
                    ${p.image ? `<div class="pf-preview-img"><img src="${p.image}" alt="${esc(p.title)}"></div>`
                              : `<div class="pf-preview-img-empty"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></div>`}
                    <div class="pf-preview-content">
                        <div class="pf-preview-header">
                            <h4 class="pf-preview-title">${esc(p.title) || 'Untitled Project'}</h4>
                            ${skillNames.length ? `<div class="pf-preview-skills">${skillNames.map(n => `<span class="pf-mini-skill">${esc(n)}</span>`).join('')}</div>` : ''}
                        </div>
                        ${p.description ? `<p class="pf-preview-desc">${esc(truncate(p.description, 100))}</p>` : ''}
                        ${host ? `<div class="pf-preview-link"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg><span>${esc(host)}</span></div>` : ''}
                    </div>
                </div>
        ...
        function renderEdit(p) {
            const selectedSkills = (p.skill_ids || [])
                .map(id => userSkillsData.find(s => s.id === id))
                .filter(Boolean);
            ...
            <div class="pf-edit-card" id="port-${p.id}">
                    ...
                    <div class="pf-form-grid">
                        <div class="pf-form-col">
                            <div class="pf-field">
                                ...
                                <input id="title-${p.id}" type="text" class="pf-input" maxlength="${TITLE_MAX}"
                                       value="${esc(p.title)}" placeholder="Enter project title"
                            ...
                            <div class="pf-field pf-skills-dropdown">
                                ...
                                <button type="button" class="pf-skills-dropdown-btn" onclick="toggleSkillsDropdown(${p.id})">
                                    Choose skills
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg>
                                </button>
                                <div class="pf-skills-dropdown-menu" id="skills-menu-${p.id}" style="display:none;">
                                    ${userSkillsData.map(s => `
                                        <label class="pf-skill-option">
                                            <input type="checkbox" ${p.skill_ids.includes(s.id) ? 'checked' : ''} onclick="toggleSkill(${p.id}, ${s.id})"/>
                                            <span>${esc(s.name)}</span>
                                        </label>
                                    `).join('')}
                                </div>
                                <div class="pf-skills-selected">
                                    ${selectedSkills.length ? selectedSkills.map(s => `
                                        <span class="pf-skill-tag">${esc(s.name)}
                                            <button type="button" onclick="toggleSkill(${p.id}, ${s.id})">×</button>
                                        </span>
                                    `).join('') : `<div class="pf-no-skills-msg">No skills selected</div>`}
                                </div>
                            </div>
                        </div>
                ...
                <div class="pf-edit-footer">
                    <button type="button" class="pf-btn secondary" onclick="cancelEdit(${p.id})">Cancel</button>
        ...
        function addPortfolio() {
            ...
            portfolios.unshift({
                ...
                image_path:'', image_disk:'public', skill_ids:[], db_id:null, position:0
            });
            editingId = id;
            render();
        }
        window.editPort   = (id) => { editingId = id; render(); };
        window.removePort = (id) => { if (confirm('Remove this project?')) { portfolios = portfolios.filter(p => p.id !== id); if (editingId === id) editingId = null; render(); } };
        window.cancelEdit = (id) => { const p = portfolios.find(x => x.id === id); if (p && !p.title && !p.description) portfolios = portfolios.filter(x => x.id !== id); editingId = null; render(); };
        window.savePort   = (id) => {
            const p = portfolios.find(x => x.id === id);
            if (!p || !p.title.trim() || !p.description.trim()) { alert('Title and description are required.'); return; }
        ...
        window.updatePort = (id, field, value) => {
            ...
            if (!p) return;
            if (field === 'title') {
                p.title = String(value).slice(0, TITLE_MAX);
                const ct = document.getElementById(`ct-title-${id}`); if (ct) ct.textContent = `${p.title.length}/${TITLE_MAX}`;
            } else if (field === 'description') {
                p.description = String(value).slice(0, DESC_MAX);
                const ct = document.getElementById(`ct-desc-${id}`); if (ct) ct.textContent = `${p.description.length}/${DESC_MAX}`;
            } else if (field === 'link') {
                p.link = String(value).trim();
            }
            updateSaveButton();
        };

        // ---------- Skills ----------
        window.toggleSkillsDropdown = (id) => {
            const menu = document.getElementById(`skills-menu-${id}`);
            if (menu) menu.style.display = (menu.style.display === 'none' || !menu.style.display) ? 'block' : 'none';
        };
        window.toggleSkill = (portId, skillId) => {
            const p = portfolios.find(x => x.id === portId);
            if (!p) return;
            const idx = p.skill_ids.indexOf(skillId);
            if (idx > -1) p.skill_ids.splice(idx, 1); else p.skill_ids.push(skillId);
            render();
            // keep menu open
            const menu = document.getElementById(`skills-menu-${portId}`);
            if (menu) menu.style.display = 'block';
        };
        document.addEventListener('click', function(e) {
            if (!e.target.closest('.pf-skills-dropdown')) {
                document.querySelectorAll('.pf-skills-dropdown-menu').forEach(menu => menu.style.display = 'none');
            }
        });
        ...
        window.handleImage = async (id, input) => {
            ...
            try {
                ...
                const p = portfolios.find(x => x.id === id);
                if (p) { p.image = dataUrl; p.image_path = ''; render(); }
            } catch (e) { console.error(e); }
            input.value = '';
        };
        window.removeImage = (id) => {
            const p = portfolios.find(x => x.id === id);
            if (p) { p.image = ''; p.image_path = ''; render(); }
        };
        ...
        // ---------- Boot ----------
        if (el.addBtn) el.addBtn.addEventListener('click', addPortfolio);
        loadExisting();
        render();
    })();
</script>
