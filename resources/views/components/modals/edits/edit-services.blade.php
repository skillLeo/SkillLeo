...
<x-modals.edits.base-modal id="editServicesModal" title="Services I Offer" size="md">
    <form id="servicesForm" method="POST" action="{{ route('tenant.services.update') }}">
        @csrf
        @method('PUT')
        ...
        {{-- hidden JSON payload for the controller --}}
...
let services = @json(array_values($services ?? []));
...
function addService() {
    ...
    if (services.some(s => s.toLowerCase() === text.toLowerCase())) {
    ...
    if (services.length === 0) {
        ...
        list.innerHTML = services.map((service, i) => `
            <div class="service-item">
                ...
                <span class="service-text">${escapeHtml(service)}</span>
