        ...
        <form id="taskStatusForm" onsubmit="submitTaskStatus(event)">
            ...
            <div class="task-modal-body">
                ...
                <div id="postponeDateField" style="display: none; margin-top: 20px;">
                    ...
                    <input type="date" id="postponed_until" name="postponed_until" class="modal-input"
                ...
                <div style="margin-top: 20px;">
                    ...
                    <textarea id="remark" name="remark" class="modal-textarea" rows="4"
                        placeholder="Describe what you did, what challenges you faced, or any important notes..." required minlength="5"
                ...
                <div style="margin-top: 20px;">
                    ...
                    <div class="upload-area" id="uploadArea">
                        ...
                        >
                        <label for="attachmentsInput" class="upload-label">
...
<script>
    ...
    function openStatusModal(taskId, action, subtaskId = null) {
        ...
        // reset upload state (this is critical to avoid stale files carrying over)
        selectedFiles = [];
        previewContainer.innerHTML = '';
        previewContainer.style.display = 'none';
        fileInput.value = ''; // now safe, element exists
        ...
        if (action === 'remark') {
            ...
            // If they clicked a quick-action like "done" or "blocked",
            // pre-check that radio
            if (action !== 'status') {
    ...
    document.getElementById('remark').addEventListener('input', function() {
        ...
        counterEl.style.color = (count > 1900) ? '#DE350B' : '#6B778C';
    });
    ...
    const fileInputEl      = document.getElementById('attachmentsInput');
    const previewContainer = document.getElementById('filePreview');
    
    // stop browser from navigating away on drop
    ...
    // take new files, validate, push to selectedFiles, render preview
    function addIncomingFiles(files) {
        files.forEach(file => {
            ...
            // don't push duplicates (same file again)
            const already = selectedFiles.find(
                f => f.name === file.name &&
                     f.size === file.size &&
            ...
            renderFilePreviewTile(file);
        });
    }
    
    function renderFilePreviewTile(file) {
            ...
            reader.onload = function(e) {
                wrapper.innerHTML = `
                    ...
                    <button type="button"
                            ...
                            onclick="removeFile('${jsStringEscape(file.name)}')">
    ...
    function removeFile(fileName) {
        ...
        const safeAttr = cssAttrEscape(fileName);
        const tile = document.querySelector(`.file-preview-item[data-file-name="${safeAttr}"]`);
        if (tile) tile.remove();
    ...
    function submitTaskStatus(event) {
        ...
        const postponedDate = document.getElementById('postponed_until')?.value || '';
        ...
        if (postponedDate) formData.append('postponed_until', postponedDate);
    
        // 👇 IMPORTANT: send each file once, using attachments[]
        ...
        if (action === 'remark') {
            ...
            url = `/{{ $username }}/manage/projects/tasks/${taskId}/subtasks/${subtaskId}/complete-final`;
            formData.append('status', 'done'); // enforce done
        } else {
            url = `/{{ $username }}/manage/projects/tasks/${taskId}/status`;
        }
    ...
    // escape string for putting inside single quotes in onclick=""
    ...
    (function injectSpinnerCss() {
        if (document.getElementById('task-modal-spinner-style')) return;
        const style = document.createElement('style');
        style.id = 'task-modal-spinner-style';
        style.textContent = `
            @keyframes spin {
